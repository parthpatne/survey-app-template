    ...
    getRadioItem(item): JSX.Element {
        ...
        return (
            <Flex className="radio-item-container" key={item.key} onClick={() => { this.props.checkedValueChanged(item.value) }}>
                ...
                <CircleIcon className="checkmark-icon checkmark-bg-icon" color={isChecked ? "brand" : "grey"} outline={!isChecked} disabled={!isChecked} />
                {isChecked ? <AcceptIcon className="checkmark-icon checkmark-tick-icon" color="white" /> : null}
                </div>
            </Flex>
        );
    }
